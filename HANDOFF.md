...
- 미국상장 4종과 국내상장 KODEX 2종의 공식 운용사 스냅샷 기반 ETF
...
1. TIGER 미국S&P500·미국나스닥100도 공식 최신 구성종목을 확보해 국내
   운용사 간 동일 지수 비교를 추가한다.
2. ETF 운용사 자료 수집을 자동화하고 현재 365일인 CI 신선도 한도를 자료별
   갱신 주기에 맞게 단축한다.
3. 방어·인컴 자산군의 상품 메타데이터와 Mock 시세를 추가해 리밸런싱 실행
   초안을 주식성 외 자산군으로 확장한다.
4. 운영 PostgreSQL과 배포 환경을 구성하고 Supabase 프로젝트에서 실제
   로그인·토큰 검증을 통합 테스트한다.
5. 계정 삭제 시 설정·모의투자 원장의 삭제 또는 보존 정책을 확정하고
   사용자 데이터 삭제 흐름을 구현한다.
...
- 공식 상품·구성종목 기준일 중 하나라도 현재보다 미래이거나 365일 넘게
