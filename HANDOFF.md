...
```text
backend pytest: 169 passed, 7 PostgreSQL tests skipped
backend Ruff (app tests): All checks passed
frontend Vitest: 20 files, 61 passed
frontend build: passed
frontend ESLint: passed
...
```text
frontend Vitest: 20 files, 60 passed
frontend build: passed
frontend ESLint: passed
git diff check: passed
```

진행 목표와 내 기본 설정 사이의 선택을 전략·포트폴리오가 공유하고, 추천 API의
목적·기간·월 투자금과 리밸런싱 제안 총액이 같은 조건을 쓰는지 검증했다. 첫해
월 투자금이 1만원 미만이면 두 화면 모두 기본 설정으로 안전하게 폴백한다.

2026-08-08 ETF 공식 스냅샷 수집 번들 검증:

```text
backend pytest: 122 passed
backend Ruff (app tests): All checks passed
ETF source parser/store/CLI/API target tests: 34 passed
git diff check: passed
live official source probe: safe abort, no candidate written
```

실제 KODEX·TIGER API 조회에서 TIGER의 새 자료를 확인했지만 KODEX가 실행일보다
미래인 2026-08-10 구성종목 기준일을 반환해 전체 갱신을 중단했다. 저장소 파일과
`/tmp` 후보를 덮어쓰지 않아 미래 기준일·부분 실패 시 원자적 보존을 확인했다.

2026-08-08 진행 목표 연결 번들 검증:

```text
frontend Vitest: 18 files, 55 passed
frontend build: passed
frontend ESLint: passed
git diff check: passed
```

저장한 진행 목표를 홈과 전략 추천 요청에 연결하고, 저장본 이름 변경·삭제·보관
개수 제한 및 월 투자금 최소값 경계를 검증했다. 서버나 계정 간 목표 동기화는
이 변경의 범위가 아니다.
...
- 공식 상품·구성종목 기준일 중 하나라도 현재보다 미래이거나 자료별 허용 기간을
  넘기면 백엔드 테스트가 실패한다. SPY 14일, 국내 KODEX·TIGER 45일, VOO
  95일, Invesco QQQ 계열 190일을 적용한다.
- TIGER 미국S&P500(360750)과 미국나스닥100(133690)은 미래에셋
  공식 자료의 2026-07-31 기준 상위 10종목을 사용한다.
  동일 지수 KODEX와의 국내 운용사 간 비교도 같은 API·UI에서 지원한다.
- 국내 4종의 동적 공식 데이터는 `app/data/etf_official_snapshots.json`에
  분리했다. 주간 CI가 공식 API를 수집해 검토용 artifact를 만들고 저장소와
  다르면 실패한다. `check_etf_sources --update`는 모든 조회가 정상이고 기준일이
  회귀하거나 미래가 아닐 때만 후보 JSON을 원자적으로 갱신한다.
...
- 포트폴리오 화면이 공유 전략 조건으로 추천 API를 호출해 성장·인컴·방어·
  현금성 자산의 목표 비중을 가져온다. 진행 목표 적용 모드이면 목돈 목적,
  목표 기간과 첫해 월 투자금을 사용하고 아니면 사용자 기본 설정을 사용한다.
- QQQM은 성장, DGRO는 배당·인컴, SGOV는 채권·방어 자산으로 분류하며 그 밖의
  현재 지원 주식과 ETF는 성장 자산으로 본다.
- 다음 월 투자금을 더한 예상 총자산에서 자산군별 목표 금액 부족분을 계산한
  후 부족분 비율대로 투자금을 배분한다. 비중이 초과된 자산군에는 0원을
  제시하며 매도나 세금·거래비용은 계산하지 않는다.
- 신규 포트폴리오는 목표 비중대로 배분하고, 원 단위 반올림 뒤에도 네 자산군
  제안액 합계가 월 투자금과 정확히 같도록 마지막 자산군에서 잔액을 맞춘다.
- 성장·인컴·방어 제안액은 각각 QQQM·DGRO·SGOV를 실행 예시로 삼아 `현재가 ×
  USD/KRW × (1+0.1% 수수료)`로 한 주 비용을 계산하고 정수 수량만 제안한다.
...
- 저장 시나리오 하나를 진행 중인 목표로 지정할 수 있다. 첫 저장본은 자동으로
  지정되며, 지정한 저장본을 삭제하면 활성 포인터도 함께 제거된다.
- 홈은 진행 목표의 적용 목표·예상 자산·달성 예상률과 첫해 납입 계획을 보여준다.
  전략 화면은 해당 목표를 목돈 마련 목적·기간·첫해 월 투자금으로 적용하며,
  사용자는 원래 내 설정으로 전환할 수 있다. 월 투자금이 전략 최소값인 1만원
  미만이면 자동 적용하지 않고 목표 계산기에서 수정을 안내한다.
- 목표 조건 또는 내 기본 설정 선택은 로컬에 유지되고 전략과 포트폴리오가 같은
  모드를 사용한다. 포트폴리오 리밸런싱의 추천 요청과 다음 월 배분액도 선택한
  기간·월 투자금을 따르며 홈에서 진행 목표 전략을 열면 목표 모드가 다시 적용된다.
