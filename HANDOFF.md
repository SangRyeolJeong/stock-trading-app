...
- 공식 운용사 스냅샷 기반 ETF 보수·상위 구성종목 중복도 비교
...
1. 현재 보유 비중과 전략 엔진 목표 비중의 차이를 계산하고 다음 월 투자금을
   리밸런싱 금액으로 제시해 포트폴리오 → 전략 실행 연결을 완성한다.
2. 국내상장 해외지수 ETF까지 비교 데이터 범위를 확장하고 공식 자료 갱신
   절차를 자동 검증한다.
