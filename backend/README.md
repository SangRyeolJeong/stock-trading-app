...
현재 투자자산, 목표 금액, 월 투자금, 기간과 예상 연 수익률을 입력받아
`GOAL-2026.08` 계산식으로 예상 자산과 목표 달성률을 계산합니다. 목표까지
부족하거나 초과하는 금액, 목표 달성에 필요한 월 투자금, 현재 계획 대비 추가
월 투자금, 연도별 예상 경로를 함께 반환합니다.

수익률 민감도는 입력값보다 2%p 낮은 경우·입력값·2%p 높은 경우를 같은 공식으로
비교할 뿐 시장 전망을 생성하지 않습니다. 현재 자산은 첫해부터 운용하고 월
투자금의 12배를 매년 말 납입하는 모델이며 세금·수수료·물가·환율은 제외합니다.
...
최대 10개 보관합니다. 링크에는 계산 결과나 인증정보를 넣지 않고 입력 조건만
담으며 현재 자산 같은 금융 입력이 포함된다는 경고를 표시합니다. 서버가 현재
`GOAL-2026.08` 엔진으로 결과를 다시 계산합니다. 저장 결과는
계정이나 서버에 동기화되지 않고 해당 브라우저에서만 불러오거나 삭제할 수 있습니다.
