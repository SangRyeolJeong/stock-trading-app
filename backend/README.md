...
ETF 비교는 KIS 시세와 분리된 `ETF-COMPARE-2026.09` 결정론적 데이터셋을
...
연금 시작 시점 API는 현재 나이부터 납입하는 경우와 기본 5년 뒤부터 같은
금액을 납입하는 경우를 비교합니다. 두 경우의 납입 원금, 예상 운용자산, 누적
세액공제와 예상 가치 차이를 반환하고, 늦게 시작해 현재 시작 시나리오를
따라잡으려면 필요한 월 납입액도 역산합니다. 연금계좌 일반 납입한도 연
1,800만원과 연금저축 세액공제 한도 연 600만원을 적용하며, 역산 납입액이
일반 납입한도를 넘는지도 함께 표시합니다.

## 투자 목표 계산 API

- `POST /api/v1/goals/simulate`

현재 투자자산, 목표 금액, 첫해 월 투자금, 기간, 예상 연 수익률, 연
물가상승률과 연 투자금 증액률을 입력받아 `GOAL-2026.08.2` 계산식으로 예상
자산과 목표 달성률을 계산합니다. 목표 금액을 만기 명목금액 또는 현재 구매력
기준으로 선택할 수 있습니다. 현재 구매력 기준이면 물가상승률로 만기 명목
목표를 계산합니다. 목표까지 부족하거나 초과하는 금액, 예상 자산의 현재가치,
목표 달성에 필요한 첫해 월 투자금, 현재 계획 대비 추가 월 투자금과 연도별
예상 경로를 함께 반환합니다.

수익률 민감도는 입력값보다 2%p 낮은 경우·입력값·2%p 높은 경우를 같은 공식으로
비교할 뿐 시장 전망을 생성하지 않습니다. 현재 자산은 첫해부터 운용하고 첫해
월 투자금의 12배를 매년 말 납입한 뒤, 다음 해 납입액을 입력 증액률만큼
늘립니다. 수익률·물가·증액률은 기간 내 일정하다고 가정하며 세금·수수료·환율과
실제 수익률·물가 변동은 제외합니다.

프론트 목표 계산기는 입력 조건을 서버 요청과 referrer에 포함되지 않는 URL
fragment로 직렬화해 같은 계산을 재현하는
공유 링크를 만들고, 사용자가 선택한 결과를 `moa-goal-snapshots-v1` 로컬 저장소에
최대 10개 보관합니다. 시나리오에는 40자 이내 이름을 붙이거나 나중에 수정할 수
있고, 이름 필드가 없는 기존 저장본도 목표 금액과 기간으로 만든 기본 이름으로
불러옵니다. 두 저장본을 선택하면 입력 조건과 저장 당시 결과를 비교합니다.
링크에는 계산 결과나 인증정보를 넣지 않고 입력 조건만 담으며 현재 자산 같은
금융 입력이 포함된다는 경고를 표시합니다. 서버가 현재 `GOAL-2026.08.2` 엔진으로
결과를 다시 계산합니다. 저장 결과는 계정이나 서버에 동기화되지 않고 해당
브라우저에서만 불러오거나 수정·삭제할 수 있습니다.

## 전략 추천 API

- `POST /api/v1/strategies/recommend`
- `POST /api/v1/strategies/explain`
...
AI 설명은 기본적으로 꺼져 있습니다. `AI_PROVIDER=openai`와 서버 전용
`OPENAI_API_KEY`를 설정하면 전략 화면에서 사용자가 명시적으로 요청할 때만
Responses API를 호출합니다. 모델은 `OPENAI_MODEL`로 바꿀 수 있으며 기본값은
`gpt-5.6`입니다. 서버가 같은 입력으로 규칙 추천을 다시 계산한 뒤 그 결과만
구조화된 설명 입력으로 전달하고, 모델이 반환한 근거 코드는 실제 규칙 엔진의
`reason_codes`에 포함되는지 검증합니다. API 키와 원문 공급자 오류는 응답이나
운영 설정 점검 출력에 포함하지 않습니다.
