...
describe('calculateRebalancingPlan', () => {
  it('uses a new contribution to fill the largest target gaps first proportionally', () => {
    ...
    expect(plan.items.find((item) => item.category === 'equity')?.suggestedContributionKrw).toBe(0);
    expect(plan.items.find((item) => item.category === 'defensive')?.suggestedContributionKrw).toBe(1_000_000);
